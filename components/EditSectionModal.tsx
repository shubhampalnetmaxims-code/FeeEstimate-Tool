...
const EditSectionModal: React.FC<EditSectionModalProps> = ({ onClose, onSave, categories, onUpdateCategories, initialData }) => {
    const [name, setName] = useState(initialData.name);
    const [description, setDescription] = useState(initialData.description);
    const [content, setContent] = useState<SectionContentItem[]>(JSON.parse(JSON.stringify(initialData.content)));
    const [selectedCategoryIds, setSelectedCategoryIds] = useState<string[]>(initialData.content.map(c => c.categoryId));
        ...
        <div className="fixed inset-0 left-64 bg-black/50 z-40 flex items-center justify-center p-4">
            ...
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-black">
                <div className="flex justify-between items-center p-4 border-b border-black">
                    <h2 className="text-xl font-bold text-black">Edit Section</h2>
                ...
                <div className="flex justify-end items-center p-4 border-t bg-gray-50">
                    ...
                    <button onClick={handleSubmit} className="px-4 py-2 text-white bg-black rounded-md hover:bg-gray-800">Save Changes</button>
                </div>
            </div>
        </div>
    );
